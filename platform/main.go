...
func runServer(host, port string) {
	...
	mux.HandleFunc("/account", s.handleAccountPage)      // signed-in landing
	mux.HandleFunc("/account/code", s.handleAccountCode) // mint a fresh setup code
	mux.HandleFunc("/logout", s.handleLogout)
	mux.HandleFunc("/signup", s.handleSignupPage)                                       // web signup: provider picker
	mux.HandleFunc("/auth/portier/start", limited(signupLimiter, s.handlePortierStart)) // begin SSO (state cookie -> portier)
	mux.HandleFunc("/auth/portier/callback", s.handlePortierCallback)                   // SSO return: verify, upsert, mint claim code
	mux.HandleFunc("/api/claim", limited(claimLimiter, s.handleClaim))                  // `mago claim`: code -> token + license
